...
class ASTMinerHandler(PluginHandler):
    ...
    def add_cwe_ids(self, dataset: pd.DataFrame, labels_df: pd.DataFrame):
        ...
        unsafe_labels_df['sfp'] = unsafe_labels_df.cwe.apply(lambda x: self.cwe_list_handler.find_primary_sfp_cluster(x, only_id=True)).astype(str)
    ...
    def convert_to_dataframe(self, results: list):
        ...
        if not all([col in df.columns for col in loc_cols]):
            raise TenetError(f"Columns not found {loc_cols} in the astminer output.")
